...
| [docs/vault-memory-pipeline.md](docs/vault-memory-pipeline.md) | End-to-end loop from a live turn to vault-side digest/briefing and back — the seam between NanoClaw (`add-message-export`, synthetic-context) and the Obsidian vault's own MBIF-crew agents (`digester`, `briefer`, `sorter`) |
...
| [docs/skill-phase-paradigm.md](docs/skill-phase-paradigm.md) | How `add-*` skills sequence apply steps into phases (Pre-flight → Apply → Wire → Build/Restart → Verify) |
