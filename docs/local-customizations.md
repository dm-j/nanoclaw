...
| Agent tools directory | A drop-in CLI tools directory for agents (`tools` command), including `setlocaltimezone`/`datetime`. Per-group timezone itself now rides upstream's own DB-backed `container_configs.timezone` (`ncl groups config update --timezone`), not a local file mechanism — see the 2026-07-27 migration in git history if you're looking for the old `.timezone`-file approach. | [container/agent-runner/src/tools/README.md](../container/agent-runner/src/tools/README.md) |
