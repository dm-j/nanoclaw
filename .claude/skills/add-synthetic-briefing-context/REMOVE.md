...
Leave `src/memory-briefing/briefer.ts`, `wikilink-cache.ts`, and `container/agent-runner/src/mcp-tools/briefing.ts` alone — those belong to `add-memory-briefing`, a separate prerequisite skill this one doesn't own. Remove that skill separately if you want it gone too (see its own REMOVE.md).
