...
describe('create_agent — guard-based authorization (wrapped delivery action)', () => {
  ...
  it('claude creator (no explicit provider) leaves the child provider unset (built-in default)', async () => {
    mockGetContainerConfig.mockReturnValue({ cli_scope: 'global' }); // parent has no explicit provider

    await runCreateAgent({ name: 'Scout', instructions: 'help' });

    // claude is the built-in default — leaving the column unset (rather than
    // stamping the literal string 'claude') means a later instance-wide
    // default change is picked up by children too.
    expect(mockInitGroupFilesystem).toHaveBeenCalledWith(
      expect.anything(),
...
describe('create_agent — model inheritance/override', () => {
  it('explicit model in payload is stamped onto the child config', async () => {
    mockGetContainerConfig.mockReturnValue({ cli_scope: 'global' });

    await runCreateAgent({ name: 'Scout', model: 'ollama-kimi-k2.6:cloud' });
  ...
  it('payload model overrides parent model', async () => {
    ...
    await runCreateAgent({ name: 'Scout', model: 'anthropic-claude-sonnet-4-6' });
