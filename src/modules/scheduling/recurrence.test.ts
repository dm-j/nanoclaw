...
// in a distinct zone to prove the group's DB-configured timezone wins.
vi.mock('../../config.js', async (importOriginal) => {
  const actual = await importOriginal<typeof import('../../config.js')>();
  return { ...actual, TIMEZONE: 'Asia/Tokyo', GROUPS_DIR: '/tmp/nanoclaw-recurrence-test/groups' };
});

// getAgentGroup is mocked so tests don't need a real central DB — used by
// run-log.ts to resolve the group folder for the auto-pause note file, not
// by handleRecurrence's own tz resolution (that's container-config.ts's
// resolveGroupTimezone, exercised for real below via the mocked getContainerConfig).
...
// resolveGroupTimezone (container-config.ts, real implementation, not mocked)
// reads the group's config row from the central DB (not initialized here).
// Default: no override → falls back to the mocked install TIMEZONE;
// individual tests set an override to test precedence.
...
beforeEach(() => {
  ...
  // resolves to a fixed group folder (used by run-log.ts's auto-pause note).
